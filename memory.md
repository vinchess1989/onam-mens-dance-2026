...
- **Key Features:**
  ...
  - **Reference Video Rehearsal Studio (`Fusion Malayalam dance.mp4`):** Dedicated video player tab accessible via header tabs (`Video • Fusion Dance • 4 Cues`) and view switcher (`Reference Video`), hosting the 05:14 reference video with **4 named song seek points**:
    ...
    Includes individual jump cue cards, interactive timeline tick markers, quick jump presets, individual section A-B repeat loops (`shanthamee 0:28-1:01`, `ivalkkoruvan 1:01-2:06`, `chettikulangara 2:06-2:50`, `njanondaliyan 2:50-3:50`), 0.01s scrub slider, micro-nudges (`±0.01s` to `±5s`), **smooth real-time video playback speed slider (0.1x to 3.0x)** with live readout badge, quick presets (0.25x-2.0x), reset 1x, and theater view.
